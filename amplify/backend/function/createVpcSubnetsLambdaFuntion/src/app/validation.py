...
def extract_user_email(event):
    """
    Extracts the 'email' value from event['requestContext']['authorizer']['claims'].
    
    Raises:
        ClientError: If the 'email' claim is missing.
    
    Returns:
        str: The user email from the event claims.
    """
    try:
        user_email = event["requestContext"]["authorizer"]["claims"]["email"]
        if not user_email:
            raise KeyError("email claim is empty")
        return user_email
    except KeyError:
        raise ClientError("AUTH_USER_MISSING", message="User email is missing from the request claims.")
